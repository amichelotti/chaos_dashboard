...
function fill_load_main_table(){
    ...
    for (var i = 0; i < ok_scraper.length; i++) {
        $("#saved_pos_" + i).html("");
        $("#saved_status_" + i).html("");
    }
      ...
      console.log("aafff " + colm_load_status);
        ...
        ok_scraper.forEach(function(scraper) {
            dataset_index = $.inArray(scraper, colm_load_element);
            if (dataset_index != -1) {
                $("#saved_pos_" + mag_index).html(colm_load_setting[dataset_index]);

                switch (colm_load_status[dataset_index]) {
                    case false:
                        $("#saved_status_" + mag_index).html('<i class="material-icons rosso">pause_circle_outline</i>');
                        break;
                    case true:
                        $("#saved_status_" + mag_index).html('<i class="material-icons verde">trending_down</i>');
                        break;
                }
